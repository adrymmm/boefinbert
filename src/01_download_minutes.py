...
Path("../data/raw/minutes").mkdir(exist_ok=True)
...
def download_minutes(month, year):
    ...
    for url in urls_to_try:
        ...
        if r.status_code == 200:
            filename = f"../data/raw/minutes/{year}-{month}.pdf"
            with open(filename, "wb") as f:
                f.write(r.content)
            print(f"(o) Done: {year} {month}")
            return True

    print(f"(x) Failed: {month} {year}")
    return False
...
meetings = [
    ...
    ("february", 2019), ("march", 2019), ("may", 2019), ("june", 2019),
